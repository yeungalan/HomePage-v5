### AUTOMATE FIELD

Topic=2026-06日文複習筆記
ID=2026-06JapaneseExam
CREATED_DATE=2026-06-15T00:00:00Z
EDITED_DATE=2026-06-15T00:00:00Z
TAG=Japanese
CATEGORY=Japanese
CATEGORY_CAPTION=Japanese
AI_SUMMARY=這是一份為期末考試整理的日文複習筆記。開首列出最需要加強的助詞（は、が、を、に、で、へ、と）及常見混淆，例如に與で、は與が。文法部分整理了～ています、～たことがあります、～てもいいです、～なければなりません、～たら、～とき、～ながら等句型，並比較くれる與もらう、教える與習う、聞く與聞こえる等容易混淆的詞語。筆記亦涵蓋可能形的變化規則、い形容詞與な形容詞的時態變化、以て形、ない形、た形及普通形接續的文型總表、丁寧形轉普通形對照表、數量詞及數字的特殊讀法與促音便、存在句、頻率副詞、詢問理由的句型，以及喜好、能力和時間相關的詞彙。

### AUTOMATE FIELD END

# 日文複習筆記

## 目前最需要加強的地方

### 1. 助詞 

| 助詞 | 用法 | 例句 |
|---|---|---|
| は | 主題 | わたしは学生です。 |
| が | 主語／新資訊 | 犬がいます。 |
| を | 受詞 | パンを食べます。 |
| に | 時間、目的地、存在 | 学校に行きます。 |
| で | 動作發生的地點 | 図書館で勉強します。 |
| へ | 方向 | 日本へ行きます。 |
| と | 一起、引用 | 友だちと話します。 |

## 常犯錯誤

- に vs で
- は vs が
- を vs が
- に vs へ
- と vs に

---

### 例子

✕ きのう学校で行きました。

○ きのう学校に行きました。

---

# 文法整理

## ～ています

表示：

- 正在做……
- 持續狀態

### 例句

- 勉強しています。
- 雨が降っています。

---

## ～ていました

表示過去正在做……

### 例句

- 昨日、勉強していました。

---

## ～たことがあります

表示曾經做過……

### 例句

- 日本へ行ったことがあります。

---

## ～てもいいです

表示可以……

### 例句

- 入ってもいいですか。

---

## ～なければなりません

表示必須……

### 例句

- 勉強しなければなりません。

口語：

- ～なきゃ
- ～なくちゃ

---

## ～たら

表示如果／當……的話

### 例句

- 時間があったら、行きます。

---

## ～とき

表示當……的時候

### 例句

- 日本へ行くとき、カメラを買いました。

---

## ～ながら

表示一邊……一邊……

### 例句

- 音楽を聞きながら勉強します。

---

# 容易混淆的表現

## くれる vs もらう

### くれる

別人給我。

- 友だちがプレゼントをくれました。

### もらう

我收到。

- 友だちにプレゼントをもらいました。

---

## 教える vs 習う

### 教える

教別人。

- 日本語を教えます。

### 習う

向別人學習。

- 日本語を習っています。

---

## 聞く vs 聞こえる

### 聞く

主動聽／詢問。

- 音楽を聞きます。

### 聞こえる

自然聽得到。

- 外の音が聞こえます。

---

## 行く vs 来る

### 行く

去。

- 学校へ行きます。

### 来る

來。

- 友だちが来ます。

---

## 住む vs 泊まる

### 住む

居住。

- 東京に住んでいます。

### 泊まる

過夜、住宿。

- ホテルに泊まりました。

---

# 可能形（能夠～）

## 第一類動詞（五段動詞）

將「う段」改成「え段＋る」

| 原形 | 可能形 |
|---|---|
| 書く | 書ける |
| 話す | 話せる |
| 読む | 読める |
| 飲む | 飲める |
| 遊ぶ | 遊べる |

### 例句

- 日本語が話せます。
- 漢字が読めます。

---

## 第二類動詞（一段動詞）

去掉「る」＋られる

| 原形 | 可能形 |
|---|---|
| 食べる | 食べられる |
| 見る | 見られる |

### 例句

- 寿司が食べられます。
- 映画が見られます。

---

## 特別第二類動詞（一段動詞）
- おきます
- います
- みます
- できます
- おります
- あびます
---

## 不規則動詞

| 原形 | 可能形 |
|---|---|
| する | できる |
| 来る | 来られる |

---

# 形容詞變化

## い形容詞

### 現在肯定

- おいしいです。

### 現在否定

- おいしくないです。

### 過去肯定

- おいしかったです。

### 過去否定

- おいしくなかったです。

---

## な形容詞

### 現在肯定

- しずかです。

### 現在否定

- しずかじゃないです。

### 過去肯定

- しずかでした。

### 過去否定

- しずかじゃなかったです。

---

# 文型接續總表

| 接續形 | 句型 | 意思・用法 |
|---|---|---|
| て形 | ください | Please do |
| て形 | います | In progress<br>I know something<br>Habitual action<br>Current state |
| ます形 | ましょうか | Offer to do something… Shall I…? |
| て形 | もいいですか | Can I do something |
| て形 | はいけません | You must not do |
| て形 | から V2 | Precondition of V2 |
| い形容詞（去い）くて<br>な形容詞（去な）で<br>N で<br>V て形 | Vて形、Vて形、Vます形 | …list of actions that have a time relationship |
| ない形 | ないでください | Please don't do |
| ない形 | なければなりません | Must do |
| ない形 | なくてもいいです | Need not do… optional |
| N／辞書形 ＋ こと | ができます | Can do |
| N／辞書形 ＋ こと | 趣味は XX です | 趣味は 辞書形 ことです（My interest is…） |
| 辞書形／Nの／時間 | まえに | …before |
| た形 | ことがあります | Have the experience of V |
| た形 | Vた形り、Vた形り します | V1, V2, and so on (no time relationship) |
| い形容詞（去い）く<br>な形容詞（去な）に<br>N に | なります | …become |
| 普通形 | とおもいます | I think… |
| 普通形 | といいます | Say… |
| 普通形 | でしょう | …, right? |
| 普通形 | …とき… | when… |
| 普通形 | …と… | if… then |
| 普通形 | V普通形 ＋ N | Modify a noun |

---

# 普通形變化表（丁寧形 → 普通形）

| 丁寧形（ます形） | 普通形 |
|---|---|
| ます | 辞書形（かく） |
| ません | ない形（かかない） |
| ました | た形（かいた） |
| ませんでした | なかった形（かかなかった） |
| い形容詞です | い形容詞 |
| い形容詞くないです | い形容詞くない |
| い形容詞かったです | い形容詞かった |
| い形容詞くなかったです | い形容詞くなかった |
| な形容詞・名詞　です | な形容詞・名詞 |
| な形容詞・名詞　じゃありません | な形容詞・名詞　じゃない |
| な形容詞・名詞　でした | な形容詞・名詞　だった |
| な形容詞・名詞　じゃありませんでした | な形容詞・名詞　じゃなかった |

---

# 一般數量詞

| 數字 | 日文 |
|---|---|
| 1 | ひとつ |
| 2 | ふたつ |
| 3 | みっつ |
| 4 | よっつ |
| 5 | いつつ |
| 6 | むっつ |
| 7 | ななつ |
| 8 | やっつ |
| 9 | ここのつ |
| 10 | とお |

## 核心概念

日文數字的讀法分為「常用發音」、「少用發音」和因語音規則而變化的「促音」。首先，來看看最基本的「常用發音」。

- 0：ゼロ / れい
- 1：いち
- 2：に
- 3：さん
- 4：よん
- 5：ご
- 6：ろく
- 7：なな
- 8：はち
- 9：きゅう
- 10：じゅう
- 0的讀法：ゼロ vs れい

## 數字「0」有兩種常見讀法：

「ゼロ」源自英文的 "zero"，而「れい」源自中文的「零」。兩者在日常生活中都經常使用，但使用情境略有不同。

電話號碼：兩種都可以使用。例如，電話號碼「03-1020-XXXX」，可以讀作「ゼロさん…」或「れいさん…」。
時間：通常使用「れい」。例如「深夜0時15分」讀作「深夜れいじ15分」。
年齡：兩種都可以使用。例如「0歳」可以說「ゼロ歳」或「れい歳」。

## 4的讀法：よん vs よ

數字「4」的常用讀音是「よん」，但在某些助数詞前會省略「ん」，只讀作「よ」。這主要是為了發音順暢。

4円（4日圓）
讀作「よえん」，而非「よんえん」。
4年生（四年級）
讀作「よねんせい」，而非「よんねんせい」。

## 補充說明

除了常用發音外，還有一些在特定情況下使用的讀法以及因連音而產生的音變。

少用的發音

部分數字在特定詞彙中有不同的讀法，這些讀法相對少見，主要出現在固定用法中。

- 0：まる 「まる」是「圓圈」的意思。在讀數字序列（如房間號碼、建築物名稱）時，有時會用「まる」來代替「ゼロ」或「れい」。
101ビル（101大樓）
讀作「いちまるいちビル」。
306号室（306號房）
讀作「さんまるろくごうしつ」。
- 4：し 「し」的讀音與「死」相同，因此在日常計數中較少使用。主要用於固定詞彙，如月份。
4月（四月）
讀作「しがつ」。
- 7：しち 主要用於時間和月份等固定用法。
7月（七月）
讀作「しちがつ」。
7時（七點）
讀作「しちじ」。
- 9：く 「く」的讀音與「苦」相同，因此較少使用。主要用於時間和月份等固定用法。
9月（九月）
讀作「くがつ」。
9時（九點）
讀作「くじ」。

## 促音便與半濁音便

當數字1、6、8、10後面接的助数詞是以「か行」、「さ行」、「た行」、「は行」開頭時，為了發音方便，數字的結尾會變成促音「っ」。這種現象源於古漢語的「入聲」，在日文中稱為「促音便」。

- 1 (いち) → いっ
- 6 (ろく) → ろっ
- 8 (はち) → はっ
- 10 (じゅう) → じゅっ / じっ
此外，當助数詞以「は行」開頭時，除了促音變，還會引發「半濁音便」，「は行」音變成「ぱ行」音。

以下是一些常見的例子：

- 1
一回（一次）：いっかい
一冊（一冊）：いっさつ
一点（一點）：いってん
一杯（一杯）：いっぱい
- 6
六回（六次）：ろっかい
六杯（六杯）：ろっぱい
- 8
八回（八次）：はっかい
八杯（八杯）：はっぱい
- 10
十回（十次）：じっかい / じゅっかい
十杯（十杯）：じっぱい / じゅっぱい

---

# 存在句

## あります

用於無生命物品。

- 本があります。

## います

用於有生命的生物。

- 猫がいます。

### 否定

- ありません。
- いません。

---

# 頻率副詞

| 日文 | 中文 |
|---|---|
| いつも | 總是 |
| よく | 常常 |
| ときどき | 有時 |
| あまり | 不太（接否定） |
| ぜんぜん | 完全不（接否定） |

### 例句

- よく映画を見ます。
- あまり勉強しません。
- ぜんぜん分かりません。

---

# 常用句型

## 詢問理由

### 問法

- どうしてですか。
- なぜですか。

### 回答

- ～からです。

### 例句

- どうして日本語を勉強しますか。
- 日本で働きたいからです。

---

# 喜好與能力

## 喜歡

- 寿司が好きです。

## 討厭

- 納豆が嫌いです。

## 擅長

- 日本語が上手です。

## 不擅長

- 歌が下手です。

---

# 時間相關詞彙

| 日文 | 中文 |
|---|---|
| 朝 | 早上 |
| 昼 | 中午 |
| 夕方 | 傍晚 |
| 夜 | 晚上 |
| 毎朝 | 每天早上 |
| 毎日 | 每天 |
| 毎週 | 每週 |
