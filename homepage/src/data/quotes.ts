...
export const QUOTES = [
  ...
  'Sometimes, letting go is the greatest courage. As long as the journey was brilliant, the journey itself is the ending. Ask me if I truly regret it — a little, yes. But the moment I turned away, that regret was scattered by the wind.',
] as const;
...
const QUOTES_ZH_HK = [
  ...
  '有的時候 放棄才是最大的勇氣 只要過程是精彩的 那這個過程就是結局 你要問我真的很遺憾嗎 確實有點 當我掉頭 那種遺憾卻又被風吹散了',
] as const;
...
const QUOTES_JA_JP = [
  ...
  '時には、諦めることこそが最大の勇気だ。過程さえ輝いていれば、その過程こそが結末になる。本当に悔いはないかと訊かれれば、少しはある。だが振り返って歩き出した瞬間、その悔いは風に吹き散らされていた。',
] as const;
