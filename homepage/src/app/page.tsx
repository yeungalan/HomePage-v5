...
import { SOCIAL_LINKS, getSocialLinksArray, type SocialPlatform } from '@/data/social'
      ...
      <TwoColumnLayout leftContainerClassName="pr-4 pl-4 mt-[120px] lg:mt-0 lg:h-[15rem] lg:h-1/2">
        <>
          ...
          <ul className="center mx-[60px] mt-8 flex flex-wrap gap-6 lg:mx-auto lg:mt-28 lg:justify-start lg:gap-4">
            {getSocialLinksArray().map(
              ([type, id], index) => {
                if (!isSupportIcon(type)) return null
                return (
                  <BottomToUpTransitionView
                    key={type}
                    delay={calculateStaggerDelay(index, ANIMATION_DELAYS.STAGGER_MS) + titleAnimateD + ANIMATION_DELAYS.TITLE_OFFSET}
